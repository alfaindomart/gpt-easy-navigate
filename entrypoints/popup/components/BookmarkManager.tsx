...
export const BookmarkManager = () => {
  ...
  return (
    <div>
      {Object.keys(nestedBookmarks).map(
        (
          siteName, //level1: render hostname menu (ChatGPT, Gemini)
        ) => (
          <div key={siteName}>
            <h2
              onClick={() => setActiveSite(activeSite === siteName ? "" : siteName)}
              className="cursor-pointer text-xl font-bold my-2"
            >
              {siteName}
            </h2>
            {activeSite === siteName && (
              <div>
                {Object.keys(nestedBookmarks[siteName]).map(
                  (
                    title, //level2: render conversation title menu from each site
                  ) => (
                    <div key={title}>
                      ...
                      {activeTitle === title && (
                        <div>
                          {nestedBookmarks[siteName][title].map(
                            (
                              bookmark, //level3: render each bookmark under each conversation title
                            ) => (
