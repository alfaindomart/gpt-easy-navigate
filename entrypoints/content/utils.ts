...
export function sortFilter(allQueries: HTMLElement[], keywordsState: string, filterSaved: boolean, sortFromNew: boolean ) {

    if (keywordsState) {
        allQueries = allQueries.filter(query => query.innerText.toLowerCase().includes(keywordsState.trim()))
    }
